  ...
  def row(idx: Index)(implicit request: DataRequest[_], messages: Messages): Option[SummaryListRow] = {
    ...
    def row(value: String = "site.notProvided"): SummaryListRow = SummaryListRowViewModel(
      ...
      actions = Seq(ActionItemViewModel(
        href = routes.ItemMaturationPeriodAgeController.onPageLoad(request.ern, request.arc, idx, CheckMode).url,
        content = "site.change",
      ...
      answer =>
        if (answer.hasMaturationPeriodAge) {
          answer.maturationPeriodAge
            .map(maturationPeriodAge => row(HtmlFormat.escape(maturationPeriodAge).toString))
            .getOrElse(row())
        } else {
          row()
        }
