...
object ChangeDestinationTypeSummary {
  def row()(implicit request: DataRequest[_], messages: Messages): SummaryListRow = {
    val destinationType = request.userAnswers.get(DestinationTypePage).get

    SummaryListRowViewModel(
      key = "destinationType.checkYourAnswersLabel",
      value = ValueViewModel(s"destinationType.$destinationType"),
      actions = Seq()
    )
  }
}
