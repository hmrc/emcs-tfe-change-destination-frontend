...
import controllers.sections.consignee.routes
    ...
    request.userAnswers.get(ConsigneeExportEoriPage).map { answer =>
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel(
            content = "site.change",
            href = routes.ConsigneeExportEoriController.onPageLoad(request.ern, request.arc, CheckMode).url,
            id = "changeConsigneeExportEori"
          ).withVisuallyHiddenText(messages("consigneeExportEori.change.hidden"))
        )
      )
    }
}
