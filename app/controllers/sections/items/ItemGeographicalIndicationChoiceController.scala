...
class ItemGeographicalIndicationChoiceController @Inject()(
  ...
  private def renderView(status: Status, form: Form[_], idx: Index, mode: Mode)
    ...
    withGoodsTypeAsync(idx) { goodsType =>
      Future.successful(status(view(
        form = form,
        action = routes.ItemGeographicalIndicationChoiceController.onSubmit(request.ern, request.arc, idx, mode),
        goodsType = goodsType
      )))
    }
  }
