...
class ItemWineGrowingZoneController @Inject()(
  ...
  def onPageLoad(ern: String, arc: String, idx: Index, mode: Mode): Action[AnyContent] =
    authorisedDataRequestAsync(ern, arc) { implicit request =>
      validateIndexAsync(idx) {
        renderView(Ok, fillForm(ItemWineGrowingZonePage(idx), formProvider()), idx, mode)
      }
    }
    ...
    authorisedDataRequestAsync(ern, arc) { implicit request =>
      validateIndexAsync(idx) {
        formProvider().bindFromRequest().fold(
          ...
          saveAndRedirect(ItemWineGrowingZonePage(idx), _, mode)
        )
      }
    }
  ...
  private def renderView(status: Status, form: Form[_], idx: Index, mode: Mode)(implicit request: DataRequest[_]): Future[Result] =
    Future.successful(status(view(
      form = form,
      action = routes.ItemWineGrowingZoneController.onSubmit(request.ern, request.arc, idx, mode)
    )))
