...
class TransportUnitsReviewController @Inject()(
                                                ...
                                                formProvider: TransportUnitsReviewFormProvider,
                                                val controllerComponents: MessagesControllerComponents,
                                                view: TransportUnitsReviewView,
                                                helper: TransportUnitsAddToListHelper
  ...
  def onSubmit(ern: String, arc: String): Action[AnyContent] =
    authorisedDataRequestWithUpToDateMovementAsync(ern, arc) { implicit request =>
      formProvider().bindFromRequest().fold(
        ...
        saveAndRedirect(TransportUnitsReviewPage, _, NormalMode)
      )
    }

  private def renderView(status: Status, form: Form[_])(implicit request: DataRequest[_]): Future[Result] =
    Future(status(view(form, helper.allTransportUnitsSummary(onReviewPage = true), routes.TransportUnitsReviewController.onSubmit(request.ern, request.arc))))
}
