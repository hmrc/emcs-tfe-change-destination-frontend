...
class DocumentsCheckAnswersController @Inject()(
  ...
  def onPageLoad(ern: String, arc: String): Action[AnyContent] = authorisedDataRequest(ern, arc) { implicit request =>
    Ok(view(checkAnswersHelper.summaryList()))
  }

  def onSubmit(ern: String, arc: String): Action[AnyContent] = authorisedDataRequest(ern, arc) { implicit request =>
