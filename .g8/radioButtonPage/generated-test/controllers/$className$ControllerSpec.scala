...
class $className$ControllerSpec extends SpecBase with MockUserAnswersService {
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in new Test(Some(
      emptyUserAnswers.set($className$Page, $className$.values.head)
    )) {
      val result = controller.onPageLoad(testErn, testDraftId, NormalMode)(request)

      status(result) mustEqual OK
      contentAsString(result) mustEqual view(form.fill($className$.values.head), NormalMode)(dataRequest(request, userAnswers.get), messages).toString
    }
    ...
    "must redirect to the next page when valid data is submitted" in new Test(Some(emptyUserAnswers)) {
      ...
      val result = controller.onSubmit(testErn, testDraftId, NormalMode)(request.withFormUrlEncodedBody(("value", $className$.values.head.toString)))
