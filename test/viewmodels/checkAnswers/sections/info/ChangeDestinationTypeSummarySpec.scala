...
class ChangeDestinationTypeSummarySpec extends SpecBase {

  "ChangeDestinationTypeSummary" - {
    ".row" - {
      Seq(ChangeDestinationTypeMessages.English).foreach { implicit messagesForLanguage =>
        ...
        MovementScenario.values.foreach { movementScenario =>
          ...
          s"should return a summary list row for $movementScenario" in {
            ...
            ChangeDestinationTypeSummary.row() mustBe SummaryListRowViewModel(
