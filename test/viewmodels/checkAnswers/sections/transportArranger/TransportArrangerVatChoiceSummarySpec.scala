      ...
      s"when being rendered in lang code of '${messagesForLanguage.lang.code}'" - {
        ...
        "when TransportArranger is NOT GoodsOwner or Other" - {
          ...
          "must output None" in {
            ...
            TransportArrangerVatChoiceSummary.row(onReviewPage = false) mustBe None
          }
        }
        ...
        "when TransportArranger is GoodsOwner or Other" - {
          ...
          "when there's no answer" - {
            ...
            "must output the expected data (from IE801)" in {
              ...
              TransportArrangerVatChoiceSummary.row(onReviewPage = true) mustBe
                Some(
          ...
          "when there's an answer" - {
            ...
            "must output the expected row (answer is 'Yes')" in {
              ...
              TransportArrangerVatChoiceSummary.row(onReviewPage = false) mustBe
                Some(
                  SummaryListRowViewModel(
                    key = messagesForLanguage.cyaChoiceLabel,
                    value = Value(Text(messagesForLanguage.yes)),
