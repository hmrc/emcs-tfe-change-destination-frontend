...
class FakeUserAnswersConnector extends UserAnswersConnector with BaseFixtures {
  override def get(ern: String, arc: String)
                  (implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Either[ErrorResponse, Option[UserAnswers]]] =
    Future.successful(Right(None))

  override def put(userAnswers: UserAnswers)
                  (implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Either[ErrorResponse, UserAnswers]] =
    Future.successful(Right(emptyUserAnswers))

  override def delete(ern: String, arc: String)
