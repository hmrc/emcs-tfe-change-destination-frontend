    ...
    Seq(DestinationWarehouseExciseMessages.English).foreach { messagesForLanguage =>
      ...
      s"when being rendered in lang code of '${messagesForLanguage.lang.code}'" - {
       ...
       lazy val view = app.injector.instanceOf[DestinationWarehouseExciseView]
        val form = app.injector.instanceOf[DestinationWarehouseExciseFormProvider].apply(MovementScenario.UkTaxWarehouse.NI, ChangeConsignee)

        implicit val doc: Document =
          Jsoup.parse(view(
            form = form,
            onSubmitCall = controllers.sections.destination.routes.DestinationWarehouseExciseController.onSubmit(request.ern, request.arc, NormalMode)
          ).toString())
